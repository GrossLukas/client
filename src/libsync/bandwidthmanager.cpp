 ...
 *
 * Re-introduced for owncloud.online (recovered from before DC-45 #12171 which
 * removed bandwidth limiting). Only the include of the (since split) upload code
 * was adjusted to uploaddevice.h; the logic is unchanged.
 *
...
void BandwidthManager::registerDownloadJob(GETFileJob *j)
{
    ...
    // Safety net: if the job is destroyed without finishing, drop the dangling
    // pointer from the list (the normal path unregisters from GETFileJob::finished()).
    connect(j, &QObject::destroyed, this, [this](QObject *o) {
        _downloadJobList.remove(static_cast<GETFileJob *>(o));
    });
